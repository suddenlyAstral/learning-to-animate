...
"""
This file is largely the same as in the original paper's git.
chnages I've made:
1) the dataset now returns t's (and not just shader id + image)
2) added a chance to select the same shader on consecutive samples, to give more positive samples to triplet's loss
3) added documentation
4) added logging and debugging code
5) removed mixing sampler for clearer code
"""


def print_every_ten_seconds_queue_size(queue):
    """
    add when debugging to keep track of the queue of images generated.
    can cause the program to 'While True' if you ctrl-c/SIGTERM during dataset teardown
    """
    logger = logging.getLogger(__name__)
    while True:
        logger.warning(f"queue size={queue.qsize()}, approximate full/empty={queue.full()}/{queue.empty()}")
        time.sleep(10)
...
def get_moderngl_worker_class(cls):
    ...
    class RendererWorker(cls):
        def __init__(self, results_queue,
            ...
            self.generate_extra_repeats = generate_extra_repeats
        ...
        def check_consistency(self):
            ...
            if image_to_compare is None:
                ...
                if not np.allclose(image_to_compare, generated_image, atol=1e-1):
                    print("ERROR IN THE GENERATOR!! Some samples are different when using the same parameters. "
                          "This is probably an issue with multithreading and/or pytorch + OpenGL.")
        ...
        def __render__(self):
            ...
            while True:
                ...
                # this gives triplets loss much more pairs
                # we don't want the first (arbitrarily, k=16) images to be from duplicate shaders because some are visualized
                select_new_shader = not self.generate_extra_repeats \
                                    or random.uniform(0, 1) < 0.5 \
                                    or self.n_frames_generated < 16
                ...
                # we want to return the time t the shader was sampled with
                # sadly, shadertoy and twigl have different key names for that property, and sometimes it's just missing
                # property names are kept because they are internal to modernGL
                try:
                    properties_dict = self.get_uniforms(program_i)
                    time_key = {"iTime", "t", "time"}.intersection(properties_dict.keys()).pop()
                    time_t = properties_dict[time_key].value
                except KeyError as e:
                    # sometimes t fails to update. in that case we skip to the next image
                    self.logger.warning(f"skipping due to {e}")
                    continue
                ...
                if len(pil_images) == self.n_samples:
                    ...
                    self.results_queue.put((pil_images, program_is, times_t))
                    pil_images = []
                    program_is = []
                    times_t = []
...
class ModernGLOnlineDataset(Dataset):
    def __init__(self, shader_paths,
        ...
        if n_samples != -1:
            assert n_samples % len(
                gpus) == 0, "Requested samples {} has to be divisible by number of gpus {} for simplicity".format(
                n_samples, len(gpus))
        ...
        # arguments for the generating worker(s)
        kwargs_dict = dict(results_queue=self.queue,
                           ...
                           # other valid options: '''temporal_sampling='continuous', fps=4'''
                           # or '''temporal_sampling=[i/100 for i in range(10000)]'''
                           gpu=0,
                           time_to_sleep_if_full=0.2,  # changed from 1
                           consistency_check=consistency_check,
                           consistency_check_its=consistency_check_its,
                           generate_extra_repeats=generate_extra_repeats)

        self.generation_process = []

        # uncomment to monitor queue size
        # process = mp.Process(target=print_every_ten_seconds_queue_size, kwargs=dict(queue=self.queue))
        # process.start()
    ...
    def __getitem__(self, index):
        if self.n_samples == -1:
            ...
            get_item = lambda: self.__getitem_finite__(index)

        try:
            pil_image, program_i, t = get_item()
        ...
        if self.transform is not None:
            ...
            image = pil_image
        return image, program_i, t

    def __del__(self):
        self.deconstruct()

    def deconstruct(self):
        # kill processes when the class is deleted
        print("attempting dataset deconstruction")
    ...
    def __getitem_finite__(self, index):
        """
        we generate samples as normal until we have self.n_samples, and store them in self.generated_images.
        Then we just use from that list. This maintains consistency between validation epochs
        """
        self.lock.acquire()
        if len(self.generated_images) >= self.n_samples:
            pil_image, program_i, t = self.generated_images[index]
        else:
            # generate and add sample
            pil_image, program_i, t = self.queue.get()
            self.generated_images.append((pil_image, program_i, t))

            # once we have generated self.n_samples images, stop adding more samples to queue
            if len(self.generated_images) >= self.n_samples and self.queue is not None:
                for p in self.generation_process:
                    p.terminate()
                del self.queue
                self.queue = None
