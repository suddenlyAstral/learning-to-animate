"""
This file is largely the same as in the original paper's git (https://github.com/mbaradad/shaders21k). Changes:
1) work on both windows and linux
2) sampled images now also return the t used
3) if a program fails to compile, just skip it and train without
"""
import logging
  ...
  def __init__(self, programs, n_images_to_generate=100, fps=10,
    ...
    def compile_single_program(i):
      ...
      try:
        ...
        if "#version 300" in frag_shader and platform.system() == "Linux":
          # twiGL shaders fail to compile on colab but succeed locally
          # changing their version corrupts some shaders but others work ok
          frag_shader = frag_shader.replace("#version 300 es", "#version 330")
        ...
        self.logger.warning(f"Failed program: {program.get_name()} skipping")
        self.logger.debug(f"With exception: {e}")
        self.logger.log(5, f"code: {frag_shader}")  # log at an even lower importance level than debug
        return None
    ...
    successful_indices = [i for i, attempt in enumerate(compiled_info) if attempt is not None]
    compiled_info = [comp for i, comp in enumerate(compiled_info) if i in successful_indices]
    self.programs = [prog for i, prog in enumerate(self.programs) if i in successful_indices]
    ...
    if len(self.frag_shader_codes) != len(self.programs):
      assert len(self.frag_shader_codes) > 0, "failed to compile every single shader"
      self.logger.info(f"compiled {len(self.frag_shader_codes)}/{len(self.programs)} fragments")
      self.logger.warning("some shader's GLSL version was changed to avoid compilation issues. Some may be corrupted")
    else:
      self.logger.info(f"Correctly compiled all {len(self.frag_shader_codes)} fragments and prepared to start rendering.")
