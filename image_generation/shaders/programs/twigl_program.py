...
def get_twigl_program_from_shader_path(shader_path):
  ...
  fragment_name = Path(shader_path).stem  # "a/b/c.fragment" -> "c"
  shader_code = read_text_file(shader_path)
  change_code_to_mode = (os.path.sep + "codes" + os.path.sep,
                         os.path.sep + "modes" + os.path.sep)
  mode = read_text_file_lines(shader_path.replace(*change_code_to_mode))  # os-robust path swapping
